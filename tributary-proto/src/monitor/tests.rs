...
/// A half whose destination a CALLER'S fence took is consumed at once, and
/// consumed exactly as its window's expiry would consume it.
///
/// A rename into pruned ground is replaced by a widened `Rescan` before the
/// destination ever reaches this Monitor, so nothing is coming to pair with the
/// source — and an unpaired half keeps the subtree it detached for an O(1)
/// reparent, and keeps the scope's moves unsettled with it, until the pairing
/// window elapses. At the option's maximum that is a day of retained watch
/// capacity over ground the caller asked this source to stop reporting.
///
/// Revert witness: leave the half parked and both watches are still armed, the
/// departure has not been reported, and `poll_timeout` still names the window's
/// far deadline.
#[test]
fn a_consumed_pending_move_tears_its_held_subtree_down_at_once() {
  let mut m = per_dir();
  let root = live_root(&mut m, scope(1));
  let _ = drain_actions(&mut m);
  let _ = drain_events(&mut m);
  ...
  assert!(
    ...
    "staging: a watched subtree for the half to hold"
  );
  ...
  assert_eq!(
    m.poll_timeout(),
    Some(at(10) + DEFAULT_MOVE_WINDOW),
    "staging: the half is parked on its pairing window"
  );
  assert!(
    m.is_watched(w_d) && m.is_watched(w_g),
    "staging: and it is holding the subtree for a pairing that cannot arrive"
  );
  assert!(
    drain_events(&mut m).iter().all(|e| !e.kind().is_removed()),
  ...
  assert!(
    events
      .iter()
      .any(|e| e.kind().is_removed() && e.location() == &loc(&["d"])),
    "the source-side departure is reported, exactly as the expiry reports it: {events:?}"
  );
  assert!(!m.is_watched(w_d), "the held subtree is torn down at once");
  assert!(!m.is_watched(w_g), "and its descendants go with it");
  assert_eq!(
    m.poll_timeout(),
    None,
    "nothing is left parked on the window"
  );
}
