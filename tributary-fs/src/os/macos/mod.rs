...
pub(crate) fn mounts_under(root: &std::path::Path) -> Option<Vec<crate::os::MountRow>> {
  ...
  for entry in &entries {
    ...
    // stack. These rows carry NO id, so two rows at one location would be two
    // IDENTICAL rows, and a table that listed a stacked mount twice on one read
    // and once on the next would compare as a change no mount made. Every row
    // here carries the same (absent) identity, so which duplicate survives is
    // immaterial — that only one does is not.
