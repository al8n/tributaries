...
///
/// The core does not need the answer. It compares whole ROWS between two
/// readings, so two rows at one location are simply two rows and each one's
/// arrival, move and departure is a difference in its own right. What a
/// visibility resolution bought — one row per location, so that a
/// location-keyed set could be diffed — is exactly what comparing rows makes
/// unnecessary. What it cost was a class of silent misses whenever the model and
/// the kernel disagreed. A hidden mount's transition now costs at most one
/// redundant recovery of ground that was already covered, which is the safe
/// direction.
///
...
///
/// # It enriches, and it MUST, because the rows it returns are COMPARED against a
/// refresh's
///
/// A consumer that diffs two readings compares whole rows. So a seed whose rows
/// carried no unique id while the next refresh's did would differ at every mount
/// that already existed at spawn — `mnt_id_unique: None` becoming `Some(u)` at one
/// location — and the difference reads as a change. On a 6.8+ host that is a
/// spurious whole-root recovery on the first authoritative refresh of every scope,
/// forever. The two readers must produce rows of the same SHAPE or the comparison
/// is between two vocabularies.
///
/// The seed read is NOT namespace-bracketed, unlike [`mount_sample`]'s, and that is
/// fine here for the same reason the seed itself is: it is best-effort, its table is
/// trust-reducing only, and a per-row read that fails degrades exactly one row —
/// which the very next authoritative refresh corrects at the cost of one recovery
/// ([`enrich_unique_ids`]).
#[cfg(all(target_os = "linux", not(miri)))]
pub(crate) fn mounts_under(root: &Path) -> Option<Vec<crate::os::MountRow>> {
  let mut rows = mounts_from_file(Path::new("/proc/self/mountinfo"), root)?;
...
pub(crate) fn mount_sample<S>(
  ...
  coherent_mount_sample(MAX_MOUNT_SAMPLE_ATTEMPTS, || {
    ...
    // two windows too, so this arm opens no hole the proven path does not already
    // carry.
...
/// Nothing has answered a UNIQUE mount id yet in this process.
const UNIQUE_MNT_ID_UNKNOWN: u8 = 0;
/// `statx(STATX_MNT_ID_UNIQUE)` answers on this kernel (6.8+): the per-row
/// enrichment is worth its syscall.
const UNIQUE_MNT_ID_PRESENT: u8 = 1;
/// The mask bit is ABSENT (4.11–6.7): no read on this host will ever answer one,
/// so the enrichment is pure waste and is skipped.
const UNIQUE_MNT_ID_ABSENT: u8 = 2;

/// Whether this kernel has a unique mount id at all, settled once and never
/// revised — the sibling of [`MOUNT_ID_TIER`], and held to exactly its
/// discipline.
///
/// The mask miss is a property of the KERNEL, not of the call, and a kernel
/// cannot lose the bit at runtime, so there is no downgrade path to represent.
/// `Relaxed` throughout: the cell publishes no other memory, and a racing pair of
/// first reads both observe the same kernel and so agree on what they write.
static UNIQUE_MNT_ID_TIER: std::sync::atomic::AtomicU8 =
  std::sync::atomic::AtomicU8::new(UNIQUE_MNT_ID_UNKNOWN);

/// Whether a caller should still ASK for a unique mount id, or may skip the read
/// because a previous one already learned this kernel has no bit to ask for.
/// PURE over the tier cell.
fn unique_mnt_id_possible_in(tier: &std::sync::atomic::AtomicU8) -> bool {
  tier.load(std::sync::atomic::Ordering::Relaxed) != UNIQUE_MNT_ID_ABSENT
}

/// Records what the FIRST read that ANSWERED learned (PURE over the tier cell).
///
/// A `statx` that FAILED settles nothing, exactly as the fdinfo tier's does: a
/// permission refusal or a vanished mountpoint says nothing about whether this
/// kernel has the bit, and pinning `Absent` on one would retire the non-recycled
/// id for the life of the process on a host that has it.
fn settle_unique_mnt_id_tier_in(tier: &std::sync::atomic::AtomicU8, answered: bool) {
  use std::sync::atomic::Ordering;
  let settled = if answered {
    UNIQUE_MNT_ID_PRESENT
  } else {
    UNIQUE_MNT_ID_ABSENT
  };
  let _ = tier.compare_exchange(
    UNIQUE_MNT_ID_UNKNOWN,
    settled,
    Ordering::Relaxed,
    Ordering::Relaxed,
  );
...
/// [`enrich_unique_ids`] over an injected tier and an injected per-row read —
/// PURE over both, so the leg a 6.8+ host can never take is the one a cell can
/// still drive.
///
/// The whole of the memo is the guard: on a kernel with no unique id EVERY row
/// answers `None`, so the loop is one wasted `statx` per mount under the root per
/// refresh forever. Skipping it is not an optimisation of a rare path — a
/// pre-6.8 container host is exactly where the row count is largest.
///
/// The guard is asked ONCE, before the loop rather than inside it, so the read
/// that SETTLES the tier is paid for by a whole refresh rather than by one row.
/// That costs one full pass on such a host and nothing after it; making the loop
/// re-ask per row would save that single pass and put a load on the hot path of
/// every host that DOES have the id, which is the one this exists to serve.
fn enrich_unique_ids_in(
  tier: &std::sync::atomic::AtomicU8,
  rows: &mut [crate::os::MountRow],
  mut read: impl FnMut(&std::path::Path) -> Option<u64>,
) {
  if !unique_mnt_id_possible_in(tier) {
    return;
  }
  for row in rows {
    row.mnt_id_unique = read(&row.location);
  }
}

/// Fills each row's [`MountRow::mnt_id_unique`](crate::os::MountRow::mnt_id_unique)
/// — the never-recycled id a row comparison rests on where the kernel has one.
///
/// # Why it is here and not in the parser
///
/// `/proc/self/mountinfo` does not carry it, so it is a `statx` PER ROW rather
/// than a field. Two things follow. It cannot live in [`parse_mountinfo`], which
/// is pure and is exercised on every host against byte fixtures; and it must be
/// taken INSIDE the same window as the table read, which is why it runs here —
/// [`mount_sample`]'s bracket holds an open `/proc/self/mountinfo` fd across both
/// halves and rejects the whole reading when the mount namespace moved, so the
/// ids and the rows they belong to describe ONE moment. A read taken outside that
/// bracket could pair a departed row with the unique id of whatever replaced it,
/// which is the exact confusion the unique id exists to end.
///
/// # A per-row failure degrades that row and nothing else
///
/// Every leg answers `None` the same way — a kernel below 6.8 leaves the mask bit
/// unset, a mountpoint that vanished between the parse and this read fails the
/// `statx`, a hardened `/proc` refuses it — and none of them invalidates the
/// sample: a row whose id could not be read is keyed by its legacy id exactly as
/// it was before this existed. What a KEY CHANGE across two reads costs is one
/// cover at that location (a departure plus an arrival, deduplicated), which is
/// the over-covering direction.
///
/// # Cost, and the two things that bound it
///
/// One `statx` per row STRICTLY UNDER the watched root, per authoritative
/// refresh. [`parse_mountinfo`] has already discarded every row outside it, so
/// this is bounded by the mounts the scope actually watches rather than by the
/// namespace — which is what keeps a thousand-row container host from paying a
/// thousand syscalls per tick for mounts no scope can see.
///
/// And it is paid only where there is something to read. On a kernel below 6.8
/// every answer is `None`, so the loop would be pure waste for the life of the
/// process; [`UNIQUE_MNT_ID_TIER`] settles that question on the first read that
/// answers and the enrichment is skipped from then on. The root's own token
/// ([`root_mnt_unique_id`], called once per refresh for
/// [`RootIncarnation`](crate::os::RootIncarnation)) settles it too, so a scope
/// whose rows are all skipped still learns the answer.
#[cfg(all(target_os = "linux", not(miri)))]
fn enrich_unique_ids(rows: &mut [crate::os::MountRow]) {
  enrich_unique_ids_in(&UNIQUE_MNT_ID_TIER, rows, root_mnt_unique_id);
}
...
pub(crate) fn root_mnt_unique_id(root: &Path) -> Option<u64> {
  ...
  // The read ANSWERED, so what it saw about the mask is a fact about this kernel:
  // settle the tier for every later reader (the per-row enrichment above skips
  // itself once this says the bit is absent). Only a successful `statx` reaches
  // here — the `Err` arms return above without settling, because a failed read
  // says nothing about the bit.
  let present = stx.stx_mask & MNT_ID_UNIQUE != 0;
  settle_unique_mnt_id_tier_in(&UNIQUE_MNT_ID_TIER, present);
  present.then_some(stx.stx_mnt_id)
}
...
///
/// That matters above the seams: with no id the scope's descent frame is the
/// device belt alone, so a `mount --bind` of a same-superblock directory is not a
/// boundary any fence can see. Before the fdinfo tier that was the standing state
/// of every 4.11–5.7 host; now it has no trigger on an ordinary mount.
///
...
/// (both read `real_mount(path->mnt)->mnt_id`), so the two share one id space and
/// a host may answer through either without splitting it.
///
