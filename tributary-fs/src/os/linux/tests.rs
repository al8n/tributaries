...
/// A kernel with no `STATX_MNT_ID_UNIQUE` pays for the per-row enrichment ONCE and
/// never again: the tier settles `Absent` and the loop is skipped whole.
///
/// The read is one `statx` per mount under the watched root per authoritative
/// refresh, and on a 4.11–6.7 host every one of them answers `None` — so without
/// the memo it is pure waste, forever, on exactly the hosts where the row count is
/// largest (a pre-6.8 container host). The tier is the same shape and the same
/// discipline as `MOUNT_ID_TIER`: settled by the first read that ANSWERED, never
/// revised, and never settled by a read that FAILED.
///
/// Driven over an INJECTED tier and an injected read, so the leg a 6.8+ host can
/// never take needs no process-wide static to be reset — and so this cell runs on
/// every host rather than only on the one that happens to lack the bit.
///
/// MUTATION WITNESS: delete the `if !unique_mnt_id_possible_in(tier) { return; }`
/// guard from `enrich_unique_ids_in` and this FAILS at `an absent bit is asked for
/// nothing` with `left: 3, right: 0`.
#[test]
fn an_absent_unique_id_bit_skips_the_per_row_read() {
  ...
  // UNSETTLED: nothing is known yet, so every row is read.
  let tier = AtomicU8::new(super::UNIQUE_MNT_ID_UNKNOWN);
  let mut unknown = rows();
  let mut reads = 0usize;
  super::enrich_unique_ids_in(&tier, &mut unknown, |_| {
    reads += 1;
    None
  });
  assert_eq!(
    reads, 3,
    "staging: an unsettled tier asks — the first refresh is what LEARNS the answer"
  );

  // SETTLED ABSENT by a read that answered: the loop is skipped whole.
  let tier = AtomicU8::new(super::UNIQUE_MNT_ID_UNKNOWN);
  super::settle_unique_mnt_id_tier_in(&tier, false);
  let mut absent = rows();
  let mut reads = 0usize;
  super::enrich_unique_ids_in(&tier, &mut absent, |_| {
    reads += 1;
    Some(7)
  });
  assert_eq!(
    reads, 0,
    "an absent bit is asked for nothing: the mask miss is a property of the \
     KERNEL, so paying a syscall per row per refresh to re-learn it is waste that \
     grows with the namespace"
  );
  assert!(
    absent.iter().all(|row| row.mnt_id_unique.is_none()),
    "and the rows keep the legacy key they were parsed with"
  );

  // SETTLED PRESENT: the enrichment runs, which is the whole point of the memo
  // being a skip rather than a disable.
  let tier = AtomicU8::new(super::UNIQUE_MNT_ID_UNKNOWN);
  super::settle_unique_mnt_id_tier_in(&tier, true);
  let mut present = rows();
  let mut reads = 0usize;
  super::enrich_unique_ids_in(&tier, &mut present, |_| {
    reads += 1;
    Some(900 + reads as u64)
  });
  assert_eq!(reads, 3, "a kernel that HAS the id still answers per row");
  assert_eq!(
    present
      .iter()
      .map(|row| row.mnt_id_unique)
      .collect::<Vec<_>>(),
    vec![Some(901), Some(902), Some(903)],
    "and each row carries the id its own location answered"
  );

  // A read that FAILED settles nothing: one `EACCES` must not retire the census's
  // non-recycled key for the life of the process.
  let tier = AtomicU8::new(super::UNIQUE_MNT_ID_UNKNOWN);
  assert!(
    super::unique_mnt_id_possible_in(&tier),
    "an unsettled tier asks, which is what leaves a failed read costing nothing \
     but its own retry"
  );
}
