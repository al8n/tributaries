...
use crate::os::linux::fanotify::{Fence, fid::Fid};
...
/// The unengaged fence — neither seat, which is what every walk row outside the fence
/// cells runs under.
fn unfenced() -> Fence<'static> {
  Fence::new(std::path::Path::new("/"), &[], no_prune())
}

/// A fence carrying `exclusions` alone.
fn excluding<'a>(root: &'a std::path::Path, exclusions: &'a [std::path::PathBuf]) -> Fence<'a> {
  Fence::new(root, exclusions, no_prune())
}

/// The root's compiled prune words, for a fence that carries the OTHER seat alone.
fn prune_words(patterns: &[&str]) -> Globs {
  Globs::new(
    patterns
      .iter()
...
fn root_without_a_handle_is_a_fatal_seed_failure() {
  ...
  let result = seed_walk(missing, [0u8; 8], 0, &some_fid(1), unfenced(), None);
  assert!(
...
fn reseed_root_fid_mismatch_is_rootgone_without_seeding() {
  ...
  let result = seed_walk(&root, [0u8; 8], root_dev, &wrong, unfenced(), None);
  let _ = std::fs::remove_dir_all(&root);
  assert!(
    matches!(result, Err(WalkError::RootGone(_))),
...
fn the_seed_walk_maps_no_directory_at_or_under_an_exclusion() {
  ...
  let all = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), None)
...
/// The PRUNE seat at the walk, on a real tree — the same boundary the exclusion
/// fence holds, reached through the other seat.
///
/// A glob can be handed to no OS API at all, so a kernel-recursive mark has exactly
/// one place to honor a prune pattern: the admission map. A pruned directory that
/// gets mapped admits every event under it forever, and its live churn goes on
/// spending map entries and transport admission for a subtree the caller asked never
/// to be enumerated. This asserts the walk keeps the whole pruned subtree out — the
/// boundary directory included, the descent skipped rather than the one entry
/// dropped — while the root anchor and every sibling are seeded untouched.
///
/// The words are ROOT-RELATIVE and matched with a literal separator, which the two
/// negative names prove: `cachex` shares a byte prefix with the pattern and is
/// mapped, and `kept/cache` is mapped under an `a/cache`-shaped pattern that names
/// one place rather than every `cache` in the tree.
///
/// Revert witness: drop the prune half from `descend`'s fence and the pruned subtree
/// is seeded again, from where every event under it admits.
#[test]
fn the_seed_walk_maps_no_directory_at_or_under_a_pruned_name() {
  use std::os::unix::fs::MetadataExt;

  let root = std::env::temp_dir().join(format!("tributary-fs-prune-walk-{}", std::process::id()));
  let _ = std::fs::remove_dir_all(&root);
  std::fs::create_dir_all(root.join("a/cache/deep")).expect("create the pruned subtree");
  std::fs::create_dir_all(root.join("a/cachex")).expect("create the prefix-sharing sibling");
  std::fs::create_dir_all(root.join("kept/cache")).expect("create the same-named elsewhere");
  let Ok(meta) = std::fs::metadata(&root) else {
    let _ = std::fs::remove_dir_all(&root);
    return;
  };
  ...
  let Some(expected) = real_dir_fid(&root, [0u8; 8]) else {
    let _ = std::fs::remove_dir_all(&root);
    eprintln!("SKIP the_seed_walk_maps_no_directory_at_or_under_a_pruned_name: no handle");
    return;
  };
  let named = |entries: &[crate::os::linux::fanotify::map::SeedEntry], name: &str| {
    entries
  ...
  // Control: with NO words every directory is mapped, so the assertions below are
  // about the seat and not about the walk failing to reach these names.
  let all = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), None)
    .expect("the unpruned walk seeds the whole tree")
    .entries;
  assert!(named(&all, "cache"), "control: cache is mapped unpruned");
  assert!(
    named(&all, "deep"),
    "control: a/cache/deep is mapped unpruned"
  );

  let prune = prune_words(&["a/cache"]);
  let fenced = seed_walk(
    &root,
    [0u8; 8],
    root_dev,
    &expected,
    Fence::new(&root, &[], &prune),
    None,
  )
  .expect("the pruned walk still seeds the rest of the tree")
  .entries;
  let _ = std::fs::remove_dir_all(&root);

  assert!(
    !named(&fenced, "deep"),
    "the seat SKIPS the descent, so nothing under the pruned name is mapped either"
  );
  assert!(
    named(&fenced, "cachex"),
    "a sibling sharing a byte prefix is still mapped — a `*` never crosses a `/`"
  );
  assert!(
    named(&fenced, "kept") && named(&fenced, "cache"),
    "a root-relative pattern names ONE place: the same name elsewhere is untouched"
  );
  assert_eq!(
    fenced
      .iter()
      .filter(|e| e.name == std::ffi::OsStr::new("cache"))
      .count(),
    1,
    "exactly the pruned `a/cache` is missing, and `kept/cache` is not"
  );
  assert!(
    fenced.iter().any(|e| e.parent.is_none()),
    "the root anchor is seeded — the root itself is never prunable"
  );
}
...
fn reseed_root_fid_match_seeds_normally() {
  ...
  let result = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), None);
  let _ = std::fs::remove_dir_all(&root);
  let entries = result
...
fn subtree_fid_mismatch_is_incomplete_without_seeding() {
  ...
  let result = subtree_walk(&subtree, &wrong, [0u8; 8], root_dev, unfenced(), None);
  let _ = std::fs::remove_dir_all(&subtree);
  assert!(
...
fn subtree_fid_match_seeds_descendants() {
  ...
  let result = subtree_walk(&subtree, &subtree_fid, [0u8; 8], root_dev, unfenced(), None);
  let _ = std::fs::remove_dir_all(&subtree);
  let entries = result
    .expect(
...
fn subtree_walk_on_missing_path_is_incomplete_not_empty() {
  ...
  match subtree_walk(&missing, &subtree_fid, [0u8; 8], 0, unfenced(), None) {
...
fn seed_walk_over_the_directory_cap_is_incomplete() {
  ...
  let capped = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), Some(2));
  assert!(
  ...
  let ok = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), Some(1000));
  let _ = std::fs::remove_dir_all(&root);
...
fn a_zero_cap_is_incomplete_even_for_an_empty_root() {
  ...
  let capped = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), Some(0));
  let _ = std::fs::remove_dir_all(&root);
  assert!(
    matches!(capped, Err(WalkError::Incomplete(_))),
...
fn a_cap_of_one_seeds_exactly_the_anchor_for_an_empty_root() {
  ...
  let ok = seed_walk(&root, [0u8; 8], root_dev, &expected, unfenced(), Some(1));
  let _ = std::fs::remove_dir_all(&root);
  let entries = ok
