...
// The callback-side machinery below is driven only by the macOS backend; the
// gate mirrors that consumer's exact cfg so a build with no backend carries
// none of it, while the interleaving/protocol suites compile it everywhere
// under test (including miri).
